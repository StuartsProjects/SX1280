...
******************************************************************************************************
  ...
  This transmits a packet in implicit, fixed length mode. The LoRa settings to use are specified in
  the 'Settings.h' file.
...
boolean SendOK;
int8_t TestPower;
uint8_t TXPacketL = 13;                      //define implicit mode packet length
...
{
  uint8_t bufffersize;
  uint8_t buff[] = "Hello World!";                     //this is 13 bytes long, 12 characters and null terminator byte at end
  buff[12] = '#';                                      //replace null terminator with # so we can see it in RX

  Serial.print(F("Buffer size = "));
  Serial.println(sizeof(buff));
  Serial.print(F("Fixed Packet size = "));
  Serial.println(TXPacketL);                           //TXPacketL is the defined and fixed packet length

  SX1280LT.printASCIIPacket(buff, TXPacketL);

  digitalWrite(LED1, HIGH);

  if (SX1280LT.sendPacketLoRa(buff, TXPacketL, 1000, PowerTX, DIO1))
  {
...
void setup_LoRa()
{
  ...
  SX1280LT.setPacketParams(12, LORA_PACKET_FIXED_LENGTH, TXPacketL, LORA_CRC_ON, LORA_IQ_NORMAL, 0, 0);
...
void setup()
{
  ...
  Serial.println(F("7_SX1280LT_LoRa_Simple_TX_Implicit Starting"));
